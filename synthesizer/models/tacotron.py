...
class Tacotron():
    ...
    def initialize(self, inputs, input_lengths, embed_targets, mel_targets=None, 
        ...
        with tf.device(split_device):
            ...
            p_inputs = tf.py_function(split_func, [inputs, split_infos[:, 0]], lout_float)
            p_mel_targets = tf.py_function(split_func, [mel_targets, split_infos[:, 1]],
                                       lout_float) if mel_targets is not None else mel_targets
            p_stop_token_targets = tf.py_function(split_func, [stop_token_targets, split_infos[:, 2]],
        ...
        for i in range(hp.tacotron_num_gpus):
            with tf.compat.v1.device(tf.compat.v1.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0",
                ...
                with tf.compat.v1.variable_scope("inference") as scope:
                    ...
                    # self.embedding_table = tf.get_variaWble(
            ...
            with tf.compat.v1.device(tf.compat.v1.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0",
                ...
                with tf.compat.v1.variable_scope("loss") as scope:
                    if hp.mask_decoder:
                        ...
                        # Compute <stop_token> loss (for learning dynamic generation stop)
                        stop_token_loss = tf.reduce_mean(tf.compat.v1.nn.sigmoid_cross_entropy_with_logits(
